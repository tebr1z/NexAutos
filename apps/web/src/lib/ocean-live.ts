...
function parseSearates(raw: unknown): LiveOcean | null {
  ...
  const locName = (id: unknown) => {
    ...
    return str(row?.name) ?? str((row as { location?: { name?: string } } | undefined)?.location?.name);
  };
  ...
  const aisData = (aisBlock?.data && typeof aisBlock.data === "object" ? aisBlock.data : aisBlock) as Record<string, unknown> | undefined;
  ...
  const voyage =
    str((aisData?.last_event as Record<string, unknown> | undefined)?.voyage) ??
    str(lastEvent?.voyage);

  if (!vesselName && !pin.lat && !str(box.status)) return null;

  const origin = locName(pol?.location) ?? str((pol as { name?: string } | undefined)?.name);
  const dest = locName(pod?.location) ?? str((pod as { name?: string } | undefined)?.name);
  const current =
  ...
  const etaRaw =
    str(pod?.date) ??
    str((aisData?.discharge_port as Record<string, unknown> | undefined)?.date);
